...
class Indicators:
    ...
    def change_in_price(self) -> pd.DataFrame:
        ...
        def rsi(self, period: int, method: str = 'wilders') -> pd.DataFrame:
            ...
            # Exponential weighted moving averages for up days
            self._frame['ewma_up'] = self._price_groups['up_day'].transform(
                lambda x: np.ewm(span=period).mean()
            )

            # Exponential weighted moving averages for down days
        ...
        # TODO Add alpha functionality to algorithm
        ...
        def check_signals(self) -> Unions[pd.DataFrame, None]:
